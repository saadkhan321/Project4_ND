...
##### The distribution of sulphates is slightly positively skewed for both wine types while the log transformed plot shows it to be some what evenly distributed.
...
##### Total Suphur Dioxide being a combination of free and bound SO2 is mostly undetected in wine. Again, the red wine samples are positively skewed while white wine samples show an approximate bell shaped distribution. Skewness for red wine samples shows wide spread with multimodality in the transformed plot just like we saw for Free SO2.
...
##### Histogram for Residual Sugar shows that most of the samples have sugar level between 1-3 g/dm3. The distribution for white wine samples is positvely skewed with transformed plot showing multi-modality with peaks at around 0.2, 0.9 and 1.2 g/dm3. On the other hand, the red wine samples seemed to more evenly distributed with slight positive skewness.
...
##### Detailed histogram for Residual Sugar also shows that most of the samples have sugar level between 1 - 3 g/dm3. Plot also shows the long tale to the posiive side for white wine samples which is in accord with the multi-modality we observed in the transformed plot.
...
ggpairs(sub_wqw_ggpairs, params = c(shape = I('.'), outlier.shape = I('.')),
        axisLabels = 'internal')
...
ggpairs(sub_wqr_ggpairs, params = c(shape = I('.'), outlier.shape = I('.')),
        axisLabels = 'internal')
...
ggplot() +
  geom_point(aes(x = quality, y = alcohol), data = wqw, size = 3,
             alpha = 0.1, position = 'jitter', color = '#3288bd')
...
ggplot() +
  geom_point(aes(x = quality, y = alcohol), data = wqr, size = 3,
             alpha = 0.1, , position = 'jitter', color ='maroon')
...
##### Red wine data set has fewer observations but shows a relation between alcohol and quality. Generally, more the alcohol content better the quality rating. The summay table above is also consistent with what is observed in the scatter plot.
...
ggplot() +
  geom_point(aes(x = quality, y = sulphates), data = wqw, size = 3,
             alpha = 0.1, position = 'jitter', color = '#3288bd')
...
ggplot() +
  geom_point(aes(x = quality, y = sulphates), data = wqr, size = 3,
             alpha = 0.1, position = 'jitter', color = 'maroon')
...
ggplot() +
  geom_point(aes(x = quality, y = citric.acid), data = wqr, size = 3,
             alpha = 0.1, position = 'jitter', color = 'maroon')
...
ggplot() +
  geom_point(aes(x = density, y = residual.sugar), data = wqw, alpha = 0.1,
             size = 3, color = '#3288bd') +
  xlim(0.985, 1.005) +
  ylim(0,30)
...
ggplot() +
  geom_point(aes(x = density, y = residual.sugar), data = wqr, alpha = 0.1,
             size = 3, position = 'jitter', color = 'maroon')
...
ggplot() +
  geom_histogram(aes(x=fixed.acidity), data = wqw, alpha = 0.8,
                 color = 'grey', fill = 'white') +
  geom_histogram(aes(x=fixed.acidity), data = wqr, alpha = 0.3,
                 color = 'darkred', fill = 'red')
...
##### **NOTE:** Scales are limited to exclude outliers for better analysis of the data.

```{r echo=FALSE, Bivariate_Plots_scatter_density_fixed_acidity_White}

ggplot() +
  geom_point(aes(x = density, y = fixed.acidity), data = wqw, alpha = 0.1,
             size = 3, color = '#3288bd') +
  xlim(0.985, 1.005) +
  ylim(3,12)
...
ggplot() +
  geom_point(aes(x = density, y = fixed.acidity), data = wqr, alpha = 0.1,
             size = 3, color = 'maroon')
...
ggplot() +
  geom_histogram(aes(x=s.a.ratio), data = wqw, alpha = 0.8, color = 'grey',
                 fill = 'white')
...
##### **NOTE:** Again, scales limitation applied to exclude outliers.

```{r echo=FALSE, Bivariate_Plots_scatter_Sugar_Acid_Ratio_SO2_White}

ggplot() +
  geom_point(aes(x = s.a.ratio, y = total.sulfur.dioxide), data = wqw,
             size = 3, alpha = 0.1, , color = '#3288bd') +
  xlim(0, 3.5) +
  ylim(0,300)
...
cor.test(wqw$s.a.ratio,wqw$total.sulfur.dioxide)
...
ggplot() +
  geom_point(aes(x = density, y = combined.acidity), data = wqr, size = 3,
             alpha = 0.1, color = 'maroon')
...
##### **NOTE:** Outliers are excluded for better analysis and visual representation.

```{r echo=FALSE, Bivariate_Plot_Residual_Sugar_Combined_Acidiy_White}

ggplot() +
  geom_point(aes(x = combined.acidity, y = residual.sugar), data = wqw,
             size = 3, alpha = 0.1, color = '#3288bd') +
  xlim(3, 12) +
  ylim(0,25)
...
ggplot() +
  geom_point(aes(x = combined.acidity, y = residual.sugar), data = wqr,
             size = 3, alpha = 0.1, color = 'maroon')
...
##### **NOTE:** Chloride outliers removed for ease of visual representation and analysis.

```{r echo=FALSE, Bivariate_Plot_Sulphates_Chlorides_White}

ggplot() +
  geom_point(aes(x = sulphates, y = chlorides), data = wqw, size = 3,
             alpha = 0.1, color = '#3288bd') +
  ylim(0,0.22)
...
##### **NOTE:** For better analysis of the relationship between chlorides and sulphates, irrelavent outliers were removed from the plot.

```{r echo=FALSE, Bivariate_Plot_Sulphates_Chlorides_Red}

ggplot() +
  geom_point(aes(x = sulphates, y = chlorides), data = wqr, size = 3,
             alpha = 0.1, color = 'maroon') +
  xlim(0.3,1.5) +
  ylim(0,0.2)
...
ggplot() +
  geom_point(aes(x = quality, y = alcohol, color = taste), data = wqw,
...
ggplot() +
  geom_point(aes(x = quality, y = alcohol, color = taste), data = wqr,
             alpha = 0.8, position = 'jitter')
...
ggplot() +
  ...
  scale_color_manual(name="Quality",
                     values=c("red","orange","yellow","#ffffbf","lightgreen",
                              "green","darkgreen"))
...
acid_density_1 <- ggplot() +
  geom_density(aes(x=fixed.acidity, color = as.ordered(quality)), data = wqw,
               size = 1)

acid_density_2 <- ggplot() +
  geom_density(aes(x=volatile.acidity, color = as.ordered(quality)), data = wqw,
               size = 1)

acid_density_3 <- ggplot() +
  geom_density(aes(x=citric.acid, color = as.ordered(quality)), data = wqw,
               size = 1)

acid_density_4 <- ggplot() +
  geom_density(aes(x=fixed.acidity, color = as.ordered(quality)), data = wqr,
               size = 1)

acid_density_5 <- ggplot() +
  geom_density(aes(x=volatile.acidity, color = as.ordered(quality)), data = wqr,
               size = 1)

acid_density_6 <- ggplot() +
  geom_density(aes(x=citric.acid, color = as.ordered(quality)), data = wqr,
               size = 1)
...
##### **NOTE:** Scatter plot modified to exlcude outliers for ease of visual representation.
...
##sub_wq_cor <- combined_wq[c("fixed.acidity", "volatile.acidity", "citric.acid", "residual.sugar",                   "chlorides", "free.sulfur.dioxide" , "total.sulfur.dioxide", "density", "pH", "sulphates", "alcohol", "quality", "s.a.ratio", "combined.acidity")]
...
```{r echo=FALSE, Plot_Two}
  ...
  ggplot(aes(x = as.ordered(quality), y = alcohol, fill = as.ordered(quality)),
         data = wqw) + theme(panel.background = element_blank()) +
    geom_boxplot() + stat_summary(fun.y = mean, geom = 'point',
                                  shape = 4, color = 'darkgreen',
                                  size = 3) +
    scale_y_continuous(breaks = seq(8, 15, 1)) +
    xlab('Quality rating of White Wine Samples') +
    ylab('Alcohol content of White Wine Samples (%)') +
    ggtitle("Alcohol vs. Quality for White Wine") +
    theme(plot.title = element_text(face="bold", color='darkblue')) +
    theme(axis.text = element_text(face="bold", colour = "black")) +
    theme(axis.title.x = element_text(face="bold", color='black'),
          axis.title.y = element_text(face="bold", color='black')) +
    scale_fill_manual(name="Quality",
                      values=c("#d53e4f","#fc8d59","#fee090","#ffffbf","#e0f3f8",
                               "#91bfdb","#4575b4")) +
    scale_shape_discrete(name="") +
...
##### Plot two is taken from the bivariate section of this report. It shows the box plot representation of Alcohol vs. Quality for samples of White wine. Stat summary function is used to plot mean value of alcohol for each qualiy rating bucket denoted by a green cross inside a box plot. For lower quality ratings (3-5), mean and median alcohol levels show a drop and as we progress through higher quality buckets there is an increasing trend seen in percentage of alcohol.
...
ggplot() +
  ...
  theme(axis.title.x = element_text(face="bold", color='black'),
        axis.title.y = element_text(face="bold", color='black')) +
  scale_color_manual(name="Quality",values=c("red","orange",
...
##### Some fo the difficulties I faced while creating this report and coding in R are documented below:

##### **Adding addtional categorical variables**
...
##### **Adding Thematics and Axis lables**

##### The frequency polygons created in the univariate section were also used in the 'Plots and Summary' section. To enhance the graph in this section I had to search on how to put themes and titles to the plot in R. Adding Title and axes lables was easier by following the code given in lesson 3 in the frequency polygon section's instructor notes. Next task was to add themes and axes lables, this was difficult as I had to look for the appropriate themes and lable codes. Some time was spent on web search to find the appopriate R code to implement this. I got a lot help in this regard from the urls below:
...
##### Scatter plot and correlation coefficient for quality vs. alcohol in the bivariate section did show a positive relation between the two variables but did not reveal the variation in the trend. I recalled the R code covered for box plots in the course material of 'Data Analysis with R' and used it to plot alcohol vs. quality. At first I used the following piece of R code:

##### *ggplot(aes(x = quality, y = alcohol), data = wqw).....*

##### This created one huge box plot spanning over all qualiy rating bins. I was really confused and surprised as I thought this plotting would have been simple. Failing to figure out the exact cause for such an unusual plot, suddenly I recalled the as.ordered functionaliy I used to convert characters to ordered factors. Then using the following code I was able to create different box plots for all quality rating buckets.

##### *ggplot(aes(x = as.ordered(quality), y = alcohol), data = wqw).....*

##### **Appropriate Axis labels, Colors and Legend**

##### Axis labels, colors and legends for the regular plots and final plots of the 'Plots and Summary' section in particular were carefully chosen. xlab, ylab and ggtitle layer functionalities were used by following the examples taught in the course material. Title, y-axis and x-axis themes were set using the information provided in the links mentioned above under the  'Adding Thematics and Axis lables' header.
...
##### The color scheme for the scatter plots in particular, diverging from red to green, was inspired by cellular energy level symbols that I nomrally use at work as a telecom engineer. The following url will give an idea:
